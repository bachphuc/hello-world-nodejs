...
import { IFile, SheetData, dir_create, get_filename, get_filename_without_extension, json_read_all_sheets_from_excel, json_read_from_excel, str_to_file, text_from_file } from "../../utils/excel-utils";
...
const DataAccess_Structure_Template = text_from_file(`${__dirname}/templates/DataAccess_Structure_Template.cs`);
const DataServiceObjects_Template = text_from_file(`${__dirname}/templates/DataServiceObjects_Template.cs`);
const MakeDSO_Template = text_from_file(`${__dirname}/templates/MakeDSO_Template.cs`);
...
export function dac_generate_data_access(tableName: string, schemas: ColumnSchema[]): DAC_generate_data_access_Result{
  ...
  const dacTemplateGeneratedContent = text_from_file(`${__dirname}/templates/DAC_Template.generated.cs`);
  ...
  result.files.push({
    ...
    path: `${__dirname}/output/${tableName}/DAC_${tableName}.generated.cs`,
    content: dacTemplateGeneratedContentOutput
  })

  const dacTemplateContent = text_from_file(`${__dirname}/templates/DAC_Template.cs`);
  ...
  result.files.push({
    ...
    path: `${__dirname}/output/${tableName}/DAC_${tableName}.cs`,
    content: str
  })
...
export function get_output_filepath(tableName: string, name: string): string{
  return `${__dirname}/output/Tables/${tableName}/${name}`;
}
...
export function dac_generate_sql(tableName: string, schemas: ColumnSchema[]): DAC_generate_sql_Result{
  ...
  const templatePath = hasCreateUserColumn ? `${__dirname}/templates/sp_stores_template_with_create_user.generated.sql` : `${__dirname}/templates/sp_stores_template.generated.sql`;
...
export function dac_generate_dataServiceObject(tableName: string, schemas: ColumnSchema[]): DAC_generate_dataServiceObject_Result{
  ...
  const classTemplate = text_from_file(`${__dirname}/templates/DataServiceObjects_Class_Template.cs`);
...
export function dac_generate_conversions(tableName: string, schemas: ColumnSchema[]): DAC_generate_conversions_Result{
  ...
  const className = get_className(tableName);
  const methodTemplate = text_from_file(`${__dirname}/templates/MakeDSO_Template_Method.cs`);
...
export function dac_generate_structure(tableName: string, schemas: ColumnSchema[]): DAC_generate_structure_Result{
  ...
  const classItemTemplate = text_from_file(`${__dirname}/templates/DataAccess_Structure_Class_Template.cs`);
...
export function dac_generate_table_schema(tableName: string, schemas: ColumnSchema[]): DAC_generate_table_schema_Result{
  const template = text_from_file(`${__dirname}/templates/Table_Schema.sql`);
...
${dropTables.join("\n")}`;
  ...
  if(files.length){
    dir_create(`${__dirname}/output/${outputFolder}`);
    dir_create(`${__dirname}/output/${outputFolder}/DAC`);
    dir_create(`${__dirname}/output/${outputFolder}/DataServiceObjects`);
    dir_create(`${__dirname}/output/${outputFolder}/sql`);
    dir_create(`${__dirname}/output/${outputFolder}/stores`);

    files.forEach(e => {
      if(e.content){
        if(/\.sql$/.test(e.name) && /^sp_/.test(e.name)){
          str_to_file(`${__dirname}/output/${outputFolder}/stores/${e.name}`, e.content);
        }
        else if(/\.sql$/.test(e.name)){
          str_to_file(`${__dirname}/output/${outputFolder}/sql/${e.name}`, e.content);
        }
        else if(/^DAC_/.test(e.name)){
          str_to_file(`${__dirname}/output/${outputFolder}/DAC/${e.name}`, e.content);
        }
        else{
          str_to_file(`${__dirname}/output/${outputFolder}/DataServiceObjects/${e.name}`, e.content);
        }
