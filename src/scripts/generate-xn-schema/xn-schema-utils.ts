...
export interface XnDBObjectSchema{
  ...
  properties: string,
  storeName?: string,
  functionParams?: {[key: string]: DBType | string},
...
function build_dataaccess_method(options: XnDBObjectSchema, properties: PropertyItem[]){
  const isDebug = false;
  const text = properties.map((e, i) => 
    !isDebug ? build_parse_item(options, e) :
    `
  try
  {
    ${build_parse_item(options, e)}
  }
  catch(Exception e){
    objEventLog.Write($"${options.functionName} exception column=${i}. Message={e.Message}");
  }
  `
  ).join("\n");
