...
const now = new Date();
const DATE_STR = `${now.getFullYear()}-${now.getMonth() > 8 ? now.getMonth() + 1 : `0${now.getMonth() + 1}`}-${now.getDate() > 9 ? now.getDate() : `0${now.getDate()}`}`;
const adyenSuccessFilePath = `${__dirname}/data/${DATE_STR}/Adyen-success-03_11_2023_9_32_AM.xlsx`;
const fixpaymentInputFilePath = `${__dirname}/data/${DATE_STR}/Prod_PaymentFails_02-11-03-11.xlsx`;
...
function process_payment_sessions(sessions: HotelPaymentSession[]){
  ...
  str_to_file(`${__dirname}/output/${DATE_STR}/update-pending-payment.sql`, sqls.join("\n\n"));
  str_to_file(`${__dirname}/output/${DATE_STR}/check-pending-payment-update-status.sql`, checkSql);
}
...
function build_update_session_sql(session: HotelPaymentSession): string{
  ...
  return `update RPM_Hotel_Payment_Session SET Update_DateTime = GETUTCDATE(), Status = 1, Waiting_Provider_Notify = 1, Provider_Response_Text = '${adyenResponseStr}', Hotel_Payment_Method_RefID = '${paymentMethod}', Provider_Response_Reason = '${DATE_STR} Fixed data' where ID = ${session.id} AND Payment_Transaction_RefID = '${session.payment_transaction_refid}' and Status not in (1, 2, 3, 5);
  GO`;
}
